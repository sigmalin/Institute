...
[CustomEditor(typeof(StochasticTexture2D)), CanEditMultipleObjects]
public class StochasticTexture2DEditor : Editor
{
    StochasticTexture2D[] targetAssets;

    public void OnEnable()
    {
        Object[] monoObjects = targets;
        targetAssets = new StochasticTexture2D[monoObjects.Length];
        for (int i = 0; i < monoObjects.Length; i++)
        {
            targetAssets[i] = monoObjects[i] as StochasticTexture2D;
        }
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if ((StochasticTexture2D.TextureType)serializedObject.FindProperty("type").enumValueIndex == StochasticTexture2D.TextureType.Color)
        ...
        if (targetAssets[0].type == StochasticTexture2D.TextureType.Normal && targetAssets[0].compressionQuality != StochasticTexture2D.CompressionLevel.HighQuality)
    ...
    private void CopyInputTextureImportType(StochasticTexture2D target)
    {
        ...
        switch (inputImporter.textureType)
        {
            case TextureImporterType.NormalMap:
                target.type = StochasticTexture2D.TextureType.Normal;
                break;
            default:
                target.type = StochasticTexture2D.TextureType.Color;
                break;
        }
    }

    private bool UnappliedSettingChanges(StochasticTexture2D target)
    {
    ...
    private static void PreprocessData(StochasticTexture2D target)
    {
        ...
        // Init progress bar
        stepCounter = 0;
        totalSteps = (target.type != StochasticTexture2D.TextureType.Other ? 4 : 0) + (target.type != StochasticTexture2D.TextureType.Other ? 9 : 12) + 1;
        ...
        if (target.type != StochasticTexture2D.TextureType.Other)
        ...
        if ((target.type == StochasticTexture2D.TextureType.Color && target.includeAlpha == true) || target.type == StochasticTexture2D.TextureType.Other)
    ...
    static void FinalizePrecomputedTextures(ref TextureFormat inputFormat, StochasticTexture2D target, ref TextureData Tinput, ref TextureData invT)
    {
        ...
        target.Tinput.Apply();
        if (target.compressionQuality != StochasticTexture2D.CompressionLevel.None)
        {
            if(target.compressionQuality == StochasticTexture2D.CompressionLevel.HighQuality)
        ...
        {
            target.material = new Material(Shader.Find("Stochastic/Stochastic"));
            AssetDatabase.AddObjectToAsset(target.material, target);
        }
        target.material.name = target.input.name;
        target.material.SetVector("_CompressionScalers", target.compressionScalers);
        target.material.SetVector("_InputSize", new Vector4(Tinput.width, Tinput.height, invT.height, 0f));
        target.material.SetVector("_ColorSpaceOrigin", target.colorSpaceOrigin);
        target.material.SetVector("_ColorSpaceVector1", target.colorSpaceVector1);
        target.material.SetVector("_ColorSpaceVector2", target.colorSpaceVector2);
        target.material.SetVector("_ColorSpaceVector3", target.colorSpaceVector3);
        target.material.SetTexture("_Tex", target.Tinput);
        target.material.SetTexture("_invTex", target.invT);
    ...
    private static void ComputeCompressionScalers(StochasticTexture2D target)
    {
        target.compressionScalers = Vector4.one;
        if (target.compressionQuality != StochasticTexture2D.CompressionLevel.None && target.type != StochasticTexture2D.TextureType.Other)
        {
    ...
    private static void RescaleForCompression(StochasticTexture2D target, ref TextureData Tinput)
    {
        int channelCount = (target.type == StochasticTexture2D.TextureType.Color && target.includeAlpha == true) || target.type == StochasticTexture2D.TextureType.Other ?
            4 : 3;
