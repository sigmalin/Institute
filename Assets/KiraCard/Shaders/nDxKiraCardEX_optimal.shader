...
Shader "nDxShasder/nDxKiraCardEX Optimal" {
    Properties {
    ...
    SubShader {
        ...
        Pass {
            ...
            float4 frag(VertexOutput i, float facing : VFACE) : COLOR {
                ...
                float faceSign = isFrontFace * 2.0 - 1.0;//( facing >= 0 ? 1 : -1 );
                ...
                float3 normalLocal = CombineNormal(faceN,lerp(PlaneNormal,nisuN,_NisuNormalPower));
                normalLocal = lerp(PlaneNormal,normalLocal,isFrontFace);
...
////// Specular:
                ...
                float HdotN = dot(halfDirection,i.normalDir);
...
#if defined(USE_AMBIENT_COLOR)
                float3 faceE = lerp(float3(0.0,0.0,0.0),lerp((UNITY_LIGHTMODEL_AMBIENT.rgb*lerp(float3(0.0,0.0,0.0),Gradation,smoothstep( 0.0, 1.0, dot(lightDirection,i.normalDir) ))),mainE.rgb,mainD.a),frameD.r);
#else
                float3 faceE = lerp(float3(0.0,0.0,0.0),lerp((lerp(float3(0.0,0.0,0.0),Gradation,smoothstep( 0.0, 1.0, dot(lightDirection,i.normalDir) ))),mainE.rgb,mainD.a),frameD.r);
#endif
