...
public class WaterSimulator_PostRender : MonoBehaviour 
{
	...
	void Start () 
	{
		...
		InputRT = new RenderTexture (256,256,0,RenderTextureFormat.ARGB32);
		RT = new RenderTexture (256,256,0,RenderTextureFormat.ARGB32);
		prevRT = new RenderTexture (256,256,0,RenderTextureFormat.ARGB32);
		prevprevRT = new RenderTexture (256,256,0,RenderTextureFormat.ARGB32);
	...
	#region post render
	private void InitPostRender()
	{
		PostRT = new RenderTexture (256,256,0,RenderTextureFormat.ARGB32);
		renderWater.sharedMaterial.SetTexture(ShaderPropertyMainTex, PostRT);

		mPostCB = new CommandBuffer();
		mPostCB.name = "post";		
	}

	private void UpdatePostRender()
	{
		Matrix4x4 view = mainCam.worldToCameraMatrix;
		Matrix4x4 proj = GL.GetGPUProjectionMatrix(mainCam.projectionMatrix, false);

		mPostCB.SetRenderTarget(PostRT);
		mPostCB.ClearRenderTarget (true, true, Color.clear);

		mPostCB.SetViewProjectionMatrices(view, proj);

		mPostCB.DrawRenderer(renderWater, OutputMat, 0);

		Graphics.ExecuteCommandBuffer (mPostCB);
		mPostCB.Clear ();

		renderWater.sharedMaterial.SetTexture(ShaderPropertyWaveTex, RT);
	}
	#endregion
	...
	private void OnGUI()
	{
		if(ShowDebug)
		{
			...
			GUI.DrawTexture(new Rect(0, 1 * h, h, h), PostRT);
			GUI.Box(new Rect(0, 1 * h - StrWidth, h, StrWidth), "INPUT");
			GUI.Box(new Rect(0, 2 * h - StrWidth, h, StrWidth), "PR");
		}
	}
	#endregion
}
